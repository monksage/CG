# Decompose Code Into Graph Nodes

## Role

You are decomposing a piece of real code into graph nodes and loading them into a running CodeGraph service.

## Objective

Take the code in the "Code" section below. Analyze it and decompose into nodes following these rules:

- A node is the smallest unit of code containing at least one decision (if/try/loop with condition).
- If a piece of code has no decision — it is a "micro", not a standalone node. Inline it into the parent node.
- Pressure from above: if a node is not testable in isolation, it's too small.
- Pressure from below: if it has no decision point, it's not a node.
- Node names are verbs describing the action: `validate_jwt`, `calc_r2`, `parse_header`. What_do only.
- Each node stores clean code without imports. Imports go into the `imports` field as a JSON array.
- Fill in `accepts` and `returns` as JSON describing the contract.
- Fill in `spec_ticket` (10-15 lines: what it does, contract, edge cases) and `spec_summary` (full description).
- Set `kind` to "contour" for decision-bearing nodes, "micro" for small helpers that you include inline.
- Set `tags` as a JSON array of domain tags (e.g., ["auth", "validation"]).

After decomposition, load every node into CodeGraph via `POST /node` and create edges via `POST /edge` for all call relationships between nodes. Edge type is "calls" for direct function calls, "uses" for data dependencies.

## Context

CodeGraph API is running at `http://localhost:39051`. Endpoints:

```
POST /node              — create node (body: id, code, language, kind, accepts, returns, imports, tags, spec_ticket, spec_summary)
POST /edge              — add edge (body: source_id, target_id, edge_type)
GET  /node/{id}         — read node back
...
- Do not invent code. Only decompose what is given below.
- Do not refactor, optimize, or fix bugs in the original code. Preserve logic as-is.
- Do not create version history — just load current state.
- Each `POST /node` must succeed (2xx) before creating edges that reference it.
- If the code is in a single language, set all nodes to that language.
- Use `NO_PROXY=localhost,127.0.0.1` when making HTTP requests.

## Verification

1. After loading, `GET /graph` should return all node ids and edges.
2. Pick a leaf node and `GET /context/{id}` — verify it returns the node's code at depth 0 and neighbor summaries at depth 1.
3. Pick a central node (most edges) and `GET /context/{id}` — verify Dunbar circles: depth 0 full, depth 1 summary, depth 2 ticket, rest names only.
4. Count: total nodes loaded must match your decomposition plan. No missing, no extras.

## Report

Write report to `corporal_reports/01_decompose_and_load_report.md`. Include your decomposition plan (which functions became nodes, which became micros and why), the graph structure (nodes + edges), and verification results.

## Subagent guidance

Do this yourself. The decomposition requires understanding the whole code at once — splitting it across soldiers will lose coherence.
